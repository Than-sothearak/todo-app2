  ...
  return (
    <ul className="container-list">
      <div className="info-bar">
        ...
        <p>Complete<span>{taskComplete}</span></p>
      </div>
      {text.map((item, taskId, itemId) => (
        <div key={item.id} className="list-index">
          <div className="text-container">
            <input title="Mark as complete" className="radio" type="checkbox" onClick={()=> onComplete(itemId)}></input>
            <li >{item.text}</li>
          </div>
